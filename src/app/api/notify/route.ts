...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!postId && !postSlug) {
      return NextResponse.json({ error: '请提供文章ID或Slug' }, { status: 400 });
    }

    // 检查 Resend API Key
    if (!process.env.RESEND_API_KEY) {
      return NextResponse.json({ error: '邮件服务未配置，请设置 RESEND_API_KEY' }, { status: 500 });
    }
    ...
    if (!subscribers || subscribers.length === 0) {
      return NextResponse.json({ message: '暂无订阅者', sent: 0 }, { status: 200 });
    }
